...
Public Class ProtectClient
    ...
    ' --- Continuous pan/tilt/zoom (direction + zoom buttons) ---
    '
    ' ⚠ PLACEHOLDER ENDPOINT. Unlike goto-preset (confirmed via the
    ' unifi-ptz-better-patrol project's source), no reliable public
    ' reverse-engineering of continuous PTZ move exists anywhere I could
    ' find — it may use a different REST shape, or go over Protect's
    ' realtime WebSocket instead of plain HTTP. Expect this to 404 until
    ' you supply the real request. To find it:
    '   1. Open Protect's web UI in Chrome, open DevTools > Network,
    '      filter to Fetch/XHR
    '   2. Drag this camera's on-screen PTZ joystick control
    '   3. Note the method, path, and body of the request it fires
    '   4. Replace the path/body below with what you captured
    Public Async Function MovePtzAsync(direction As String) As Task(Of GotoResult)
        Dim targetId As String
        SyncLock _lock
            targetId = _selectedCameraId
        End SyncLock
        If String.IsNullOrEmpty(targetId) Then
            Return New GotoResult With {.Success = False, .StatusCode = 400, .Message = "No camera selected"}
        End If

        Dim body = New With {.direction = direction, .speed = 5}
        Dim response = Await SendAsync(HttpMethod.Post, $"/proxy/protect/api/cameras/{targetId}/ptz/move", body)
        Dim code = CInt(response.StatusCode)
        Return New GotoResult With {
            .Success = response.IsSuccessStatusCode,
            .StatusCode = code,
            .Message = $"{direction}: HTTP {code} (placeholder endpoint — see comments in ProtectClient.vb)"
        }
    End Function

    ' Called on button release to stop continuous movement. Same caveat as
    ' MovePtzAsync above — replace with the real stop request once captured.
    Public Async Function StopPtzAsync() As Task(Of GotoResult)
        Dim targetId As String
        SyncLock _lock
            targetId = _selectedCameraId
        End SyncLock
        ...
        Dim body = New With {.direction = "stop"}
        Dim response = Await SendAsync(HttpMethod.Post, $"/proxy/protect/api/cameras/{targetId}/ptz/move", body)
        Dim code = CInt(response.StatusCode)
        Return New GotoResult With {
            .Success = response.IsSuccessStatusCode,
            .StatusCode = code,
            .Message = $"stop: HTTP {code} (placeholder endpoint)"
        }
    End Function
